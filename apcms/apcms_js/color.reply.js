...
bbclose='[/c]'
	if ((clientVer >= 4) && is_ie && is_win) {
		...
		if (theSelection) {
			...
			MM_findObj('reply_message').focus();
			return;
		}
		}
		if (MM_findObj('reply_message').createTextRange && MM_findObj('reply_message').caretPos) {
			text = bbopen + bbclose ;
			var caretPos = MM_findObj('reply_message').caretPos;
			caretPos.text = caretPos.text.charAt(caretPos.text.length - 1) == ' ' ? text + ' ' : text;
			MM_findObj('reply_message').focus();
			return;
		} else {
			MM_findObj('reply_message').value  += bbopen + bbclose;
			MM_findObj('reply_message').focus();
			return;
	}
}
...
for (j=0;j<6+1;j++){
  ...
  codehex = r + '' + g + '' + b;
  document.write('		<td bgColor=\"' + RGB2Hexa(r,g,b) + '\" onClick=\"insertTag(this.bgColor);lightCase(this);\" onmouseover=\"lightCase1(this);\" title=\"'+text1+ RGB2Hexa(r,g,b) +'\" width=\"'+w+'\" height=\"'+h+'\"><IMG height='+h+' src="templates/subSilver/images/spacer.gif" width='+w+' border=0></TD>\n');
	}}}
