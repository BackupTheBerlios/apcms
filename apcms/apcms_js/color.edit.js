...
bbclose='[/c]'
	if ((clientVer >= 4) && is_ie && is_win) {
		...
		if (theSelection) {
			...
			MM_findObj('new_message').focus();
			return;
		}
		}
		if (MM_findObj('new_message').createTextRange && MM_findObj('new_message').caretPos) {
			text = bbopen + bbclose ;
			var caretPos = MM_findObj('new_message').caretPos;
			caretPos.text = caretPos.text.charAt(caretPos.text.length - 1) == ' ' ? text + ' ' : text;
			MM_findObj('new_message').focus();
			return;
		} else {
			MM_findObj('new_message').value  += bbopen + bbclose;
			MM_findObj('new_message').focus();
			return;
	}
}
